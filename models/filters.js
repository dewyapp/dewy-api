                ...
                map: [
                    'function (doc, meta) {',
                        'if (meta.id.substring(0, 9) == "project::") {',
                            'emit([doc.project, doc.core, "drupal.org", doc.releases]);',
                        '}',
                        'else if (meta.id.substring(0, 6) == "site::" && doc.uid == "' + filterDoc.uid + '" && doc.enabled == "1" && ("details" in doc) && !("error" in doc.audited)) {',
                            processedRules.test,
                            'if (' + processedRules.rule + ') {',
                                ...
                                'for (module in doc.details.modules) {',
                                    'if (doc.details.modules[module].project == null) {',
                                        'emit([module, core, "local", null]);',
                                    '}',
                                    'else {',
                                        'emit([doc.details.modules[module].project, core, "local", doc.details.modules[module].version]);',
                                    '}',
                                '}',
                            '}',
                        '}',
