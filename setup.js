                ...
                from_audited_sites_by_uid: {
                        ...
                        'function (doc, meta) {',
                            'if (meta.id.substring(0, 6) == "site::" && doc.enabled == "1" && doc.audit.lastSuccessfulAudit && doc.audit.errors.length < 3) {',
                                ...
                                'for (role in doc.details.roles) {',
                                    ...
                                    '    inUse = 1;',
                                    '}',
                                    'emit([doc.uid, role], {baseurl: doc.baseurl, available: 1, inUse: inUse, users: roles[role].users, permissions: roles[role].permissions});',
                                '}',
                            '}',
                        '}'
                        ].join('\n'),
                    reduce: [
                        'function(key, values, rereduce) {',
                            'var baseurls = {}',
                            'var users = {}',
                            'var result = {baseurls: [], available: 0, inUse: 0, users: []};',
            ...
            views: {
                from_audited_sites_by_uid: {
                            ...
                            'if (meta.id.substring(0, 6) == "site::" && doc.enabled == "1" && doc.audit.lastSuccessfulAudit && doc.audit.errors.length < 3) {',
                                'for (user in doc.details.users) {',
                                    ...
                                    'if (doc.details.users[user].last_access > doc.details.users[user].created) {',
                                        ...
                                        'not_used = 0;',
                                    '}',
                                    'emit([doc.uid, user, doc.details.users[user].mail], {baseurl: doc.baseurl, available: 1, blocked: blocked, created: doc.details.users[user].created, last_access: last_access, not_used: not_used, roles: doc.details.users[user].roles});',
                                '}',
                            '}',
                        '}'
                        ].join('\n'),
                    reduce: [
                        'function(key, values, rereduce) {',
                            'var roles = {}',
                            'var result = {baseurls: [], available: 0, blocked: 0, created: 0, last_access: 0, not_used: 0, roles: []};',
                            'for(var i = 0; i < values.length; i++) {',
                                'result.available += 1',
